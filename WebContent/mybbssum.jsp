...
<body>
	...
	<div style="text-align: right; margin-right: 6%;">
		<a href="mybbs.jsp"><button type="button" class="btn btn-default"
	...
	<div style="text-align: center;">
		<div class="container">
			<%
					   ArrayList<Bbs> list = bbsDAO.getMySumList(pageNumber,userID);
						for (int i = 0; i < list.size(); i++) {
					%>
				<div class="col-xs-12 col-sm-6 col-md-3 ">
					<div class="thumbnail" style="float: left;">
						<a href="view.jsp?bno=<%=list.get(i).getBno()%>&cnt_view=<%=list.get(i).getCnt_view()%>">
						<img src="upload\<%=list.get(i).getFileRealName()%>" style="height:300px; width: 250px;"class = "asd">
						</a>
						<div class="caption">
						<h5><a href="view.jsp?bno=<%=list.get(i).getBno()%>&cnt_view=<%=list.get(i).getCnt_view()%>">
							<b><%=list.get(i).getBoard_title()%></b>
							</a></h5>
							<p><b><%=list.get(i).getWrite_id()%></b></p>
							<p><%=list.get(i).getMake_date()%>&nbsp;&nbsp;|&nbsp;&nbsp;<%=list.get(i).getCnt_view()%></p>
							<p class="glyphicon glyphicon-thumbs-up" aria-hidden="true">&nbsp;<%=list.get(i).getCnt_good() %></p> &nbsp;
							<p class="glyphicon glyphicon-thumbs-down" aria-hidden="true">&nbsp;<%=list.get(i).getCnt_bad() %></p>
						</div>
					</div>
				</div>
